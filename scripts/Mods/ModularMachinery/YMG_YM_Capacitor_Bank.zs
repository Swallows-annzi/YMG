...
MMEvents.onControllerGUIRender("YMG_YM_Capacitor_Bank", function(event as ControllerGUIRenderEvent) {
    ...
    var info as string[] = [];
    info += "§b>>§r";
    // info += "§9RF存量：§e" + map["EnergyString"].asString() + "§r";
    if(setEnergyMax)
        info += "§9RF存量：§b(§e" + FormatNumber(map["EnergyString"].asString()) + " §6RF / §b" + FormatNumber(BigInteger.Pow("10" , setEnergyMaxNumberOfDigit)) + " §6 RF§b)§r";
        // info += "§9RF存量：§b(§e" + map["EnergyString"].asString()[0] + "." + map["EnergyString"].asString()[1] + map["EnergyString"].asString()[2] + map["EnergyString"].asString()[3] + " §6 * §b10^" + (map["EnergyString"].asString().length() - 1) + " §6RF / §b10^" + setEnergyMaxNumberOfDigit + " - 1 §6 RF§b)§r";
    
    if(!setEnergyMax)
        info += "§9RF存量：§b(§e" + FormatNumber(map["EnergyString"].asString()) + " §6RF / §c无限 §6 RF§b)§r";
    info += "§9传输倍率：§e" + map["speed"].asLong() + ".0§cx§r";
    val Speed = BigInteger.Multiply(BigInteger.toString(map["speed"].asLong()), Base_RateNumber);
    // info += "§9传输速度：§e" + Speed[0] + "." + Speed[1] + Speed[2] + Speed[3] + " §6 * §b10^" + (Speed.length() - 1) + "§r";
    info += "§9传输速度：§e" + FormatNumber(Speed) + "§r";
    info += "§b>>§r";
    // info += data;
...
function FormatNumber(String as string) as string {
    ...
    if(Number > 21) {
        return String[0] + "." + String[1] + String[2] + String[3] + " §6 * §b10^" + (Number - 1);
    }

    var UnitGroup = 0;
    var HeadLength = Number;
    for i in 1 to 6 {
        if(HeadLength > 3) {
            HeadLength -= 3;
            UnitGroup = i;
    ...
    var OutputString = "";
    for i in 0 to HeadLength - 1 {
        OutputString += String[i];
    }
    OutputString += ".";
    for i in HeadLength to HeadLength + 2 {
        OutputString += String[i];
    }
    return OutputString + GetFormatUnit(UnitGroup);
}

function GetFormatUnit(UnitGroup as int) as string {
    if(UnitGroup == 1) return " §6K";
    if(UnitGroup == 2) return " §6M";
    if(UnitGroup == 3) return " §6G";
    if(UnitGroup == 4) return " §6T";
    if(UnitGroup == 5) return " §6P";
    return " §6E";
}
